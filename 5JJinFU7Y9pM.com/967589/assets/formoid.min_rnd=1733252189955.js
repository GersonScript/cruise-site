...
var isValidEmail = function (input) {
  return input.value
    ? /^([^@]+?)@(([a-z0-9]-*)*[a-z0-9]+\.)+([a-z0-9]+)$/i.test(input.value)
    : true;
};
...
formComponents.forEach(function (component) {
  var formData,
    ...
    title = component.getAttribute("data-form-title")
      ? component
      : component.querySelector("[data-form-title]"),
    ...
    if (event.target.type === "file") {
      if (event.target.files[0].size > 1000000) {
        if (typeof bootstrap !== "undefined") {
          ...
          formModal[0].querySelector(".modal-body p").innerText =
            "File size must be less than 1mb";
          formModal[0]
            .querySelector(".modal-content")
            .classList.add("alert-danger");
          formModal[0].querySelector(".modal-content").style.backgroundColor =
            "#ff9966";
          formModal.modal();
        }
  ...
  form.addEventListener("submit", function (event) {
    ...
    formData =
      ...
      Formoid.Form({
        ...
        recaptcha: form.querySelector("[data-form-captcha]")
          ? form.querySelector("[data-form-captcha]").value
          : false,
      });
    ...
    function parseInput(input) {
      return new Promise(function (resolve, reject) {
        ...
        var name =
          input.getAttribute("data-form-field") || input.getAttribute("name");
